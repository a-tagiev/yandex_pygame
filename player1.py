...
def screen_update3():
    ...
    looser.image = pygame.transform.scale(looser.image, (looser.image.get_width() // 3, looser.image.get_height() // 3))
    looser.rect = looser.image.get_rect()
    looser.rect.center = (175, 650)
    sc.blit(sf1, (0, 0))
    sc.blit(sf2, (1150, 0))
    sc.blit(ring, (550, 250))
    sc.blit(deck.image, deck.rect)
...
class Player():
    ...
    def receiving(self):
        while True:
            try:
                data = self.soc.recv(2048)  # 1024 бит - выделяемая память на сообщение
                data1 = pickle.loads(data)
                lbl = data1[0]
                if data1[0] == "hand_out":
                    # print(data1)
